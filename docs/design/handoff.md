...
- **Fines on this case** — the fine notices issued from it, and a **Draft a fine notice** button that opens the composer below with the case's address, inspector and inspection date already filled in.

Beneath the violations list sits **Fine notices**, with its own search and status chips (all / drafted / sent / unpaid / cured / escalated / waived). The composer writes the letter: who it goes to (a home picked off the roster, so the fine can be billed; a separate mailing address only through the structured fields), which notice in the ladder, the violations charged and their amounts, what was observed and the governing section behind it, the cure date, any continuing fine and administrative fee, the payment and dispute windows, and who is copied. Amounts are quick-picked from the posted fine schedule — `fee_schedule` rows in the "Fines" category, edited under Accounting → Fee schedule — so a letter quotes an amount the board adopted rather than one somebody typed. The notice level is suggested from how many notices already stand against that address, and the office can overrule it.

Drafting bills nobody. **Print the letter** opens a two-page document in its own window — the notice, then a photo exhibit built from photos read in the browser at print time — and the print dialog saves it as PDF. **Record it as sent** takes the date, the delivery method and a tracking number, and is the step that makes the fine real: an invoice is raised against the home and issued immediately so the owner's balance matches the letter in their hand, the mailing is written to the case file's mailing record, and a case still at reported or courtesy moves to notice sent. Fines are the association's revenue and are stamped `sofilakes` on the notice, the invoice and the ledger entry the payment eventually writes. A sent notice's action dropdown offers corrected, escalated to the board, and waived — waiving voids the invoice behind it, because a waived fine left on the balance is still being collected. A notice never carries a paid flag; that is the invoice's to say. An unsent draft can be discarded; a sent one never can.
